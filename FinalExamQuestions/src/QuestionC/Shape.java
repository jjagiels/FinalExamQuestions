package QuestionC;

abstract class Shape {
	private int x;
	private int y;

	Shape(int newx, int newy) {
		moveTo(newx, newy);
	}

	/**
	 * Getter for 'x'
	 * @return - returns the value for 'x'
	 */
	int getX() throws IllegalRectangle {
		if(x < 0){ throw new IllegalRectangle("Your X value is not positive!");}
		return x;
	}

	/**
	 * Getter for 'y'
	 * @return - return the value for 'y'
	 */
	int getY() throws IllegalRectangle {
		if(y < 0){ throw new IllegalRectangle("Your Y value is not positive!");}	
		return y;
			
	}

	/**
	 * Setter for 'x'
	 * @param newx - value for 'x'
	 */
	
	void setX(int newx) {
		x = newx;
	}

	/**
	 * Setter for 'y'
	 * @param newy - value for 'y'
	 */
	void setY(int newy) {
		y = newy;
	}


	/**
	 * Move shape to new coordinates
	 * @param newx - new 'x' value
	 * @param newy - new 'y' value
	 */
	void moveTo(int newx, int newy) {
		setX(newx);
		setY(newy);
	}

	/**
	 * Move shape using delta values
	 * @param deltax - delta 'x' value
	 * @param deltay - delta 'y' value
	 * @throws IllegalRectangle 
	 */
	void rMoveTo(int deltax, int deltay) throws IllegalRectangle {
		moveTo(getX() + deltax, getY() + deltay);
	}

	/**
	 * This abstract method will compel any class that inherits this class to implement this method
	 */
	abstract void draw();
	
	
	abstract public double ComputeArea();
}